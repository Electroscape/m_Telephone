...
/* For Telephone */
const byte KeypadRowPins[KEYPAD_ROWS] = { 3, 4, 5, 6 }; 	// Measure
const byte KeypadColPins[KEYPAD_COLS] = { 0, 1, 2 };    	// Control wires (alternating HIGH)
...
void loop() {
  ...
  while(inputs.digitalRead(PHONE_PICKUP_PIN)){
    ...
    while(1){
      ...
      if (!inputs.digitalRead(PHONE_PICKUP_PIN)){break;}
    }
    phone_quit();
  }
}
...
void LCD_Update(){
	static unsigned int counterLCD = 0;
        ...
        UpdateLCDAfterDelayTimer = millis();
  			UpdateLCD = false;
  			Serial.print("NoUpdateLCD "); Serial.println(counterLCD);
        LCD_TaxiArrived();
        counterLCD++;
     }
}
...
bool DFP_Init() {
  ...
  if (!myDFPlayer.begin(altSerial)) {  //Use softwareSerial to communicate with mp3. mySoftwareSerial
    ...
    wdt_enable(WDTO_8S);
    while(true){
      if (myDFPlayer.begin(altSerial)){wdt_reset(); break;}
